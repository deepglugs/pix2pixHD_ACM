...
class BaseOptions():
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--vocab_size', type=int, default=512,
                                 help='size of vocabulary for conditional network')
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            '--acm_dim', type=int, default=64,
            help='# filters in acm layer (used only with --cond)')
        self.parser.add_argument(
            '--n_self_attention', type=int, default=0, help='number of self attention layers')
    ...
    def parse(self, save=True):
        ...
        if self.opt.device == "cuda" and not torch.cuda.is_available():
            self.opt.device = 'cpu'
            print("using cpu because no cuda...")

        # set gpu ids
        if self.opt.device == "cuda" and len(self.opt.gpu_ids) > 0 and torch.cuda.is_available():
